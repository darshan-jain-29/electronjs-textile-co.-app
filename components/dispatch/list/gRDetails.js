...
function loadGRDetails() {
    ...
    clearFields();
    invoiceNumber = printingData.billNo.replace('-', '/');
    partyName = printingData.partyName1;
    brokerName = printingData.brokerName;

    connection.query("SELECT seriesValue from seriesnumber where seriesName in ('grNumber', 'rollsSeries') order by seriesName asc;", function (
        error,
        results
    ) {
        if (error) { alert(error + " : Tab - "); throw error; }
        grSeries = parseInt(results[0].seriesValue);
        rollNumber = parseInt(results[1].seriesValue);
    });

    connection.query(
        "SELECT * from dispatchrows where  invoiceNumber = '" + invoiceNumber + "';",
...
function calculateQueryMasterRequest() {
    ...
    //insert into stock rows : add total no of non zero GRs
    queryMasterRequest += parseInt(totalGRRows)
...
function saveModalGRData() {
    ...
    checkIfGrMasterExists(invoiceNumber, partyName, brokerName);
    ...
    insertNewRowsInStockRowsTable(secondaryTableData);
...
function checkIfGrMasterExists(b, p, br) {
    var myQuery = "SELECT * from grmaster where  invoiceNo = '" + b + "' and partyName = '" + p.trim() + "' and brokerName = '" + br.trim() + "' ;";
    console.log(myQuery);
    connection.query(
        myQuery,
        function (error, results) {
...
function updateGrMasterEntry() {
    ...
    var myQuery = "UPDATE grmaster set totalQty = totalQty +" + grTotalQty +
...
function saveModalSecondaryDataInGRRows() {
    ...
    for (var r = 1, n = table.rows.length - 1; r < n; r++) {
        if (parseFloat(table.rows[r].cells[7].getElementsByTagName("input")[0].value) > 0) {
            ...
            var tableShade = tableTempVal[0].trim();
            var tableColor = tableTempVal[1].trim();
...
function updateDispatchMasterTable() {
    var myQuery = "UPDATE dispatchmaster set qtyGR = qtyGR +" + grTotalQty +
...
function performUpdatesOnDispatchRowsTable(caseQueryQty, whereQuery) {
    var myQuery = "UPDATE dispatchrows set grQty = " + caseQueryQty +
        " where  " + whereQuery;
...
function insertNewRowsInStockRowsTable(data) {

    for (var i = 0; i < data.length; i++) {
        if (parseFloat(data[i].qtyGR) > 0) {
            var qualityName = data[i].qualityName;
            var shade = data[i].shade;
            var color = data[i].color;
            var isDamage = data[i].isDamage;
            var lotNo = data[i].lotNo;
            var rollNo = rollNumber;
            var gRQty = data[i].qtyGR;
            rollNumber += 1;
            ///console.log(obj);
            connection.query(
...
function updateSeriesNumber() {
    ...
    var caseQuery = "(case ";
    var whereQuery = '';
    caseQuery = caseQuery + "when (seriesName = 'grNumber' ) THEN " + grSeries + " " + "when (seriesName = 'rollsSeries' ) THEN " + rollNumber + " ";
