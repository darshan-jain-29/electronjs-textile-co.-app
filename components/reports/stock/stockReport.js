...
function viewallSalePOList() {
  document.title = "STOCK Report - Kalash Infotech";
  document.getElementById("pageHeading").innerHTML = "STOCK Report";
...
function loadDataDropdown(selectName, results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (selectName == "partyNameDropdown" && results[i].type == "parties") {
      ...
      options.innherHTML = options.text = results[i].value.toUpperCase();
      theSelect.add(options);
...
function qSel() {
  var fld = document.getElementById('qualityNameDropdown');
  var values = [];
...
function generateEditStockView() {
  document.title = "STOCK Edit Page - Kalash Infotech";
  document.getElementById("pageHeading").innerHTML = "STOCK Edit Page";
