...
function loadData() {
  ...
  connection.query(
    "SELECT * from greypomaster where status = 'COMPLETED' ORDER BY uPoNumber desc",
...
function viewAllPoList() {
  document.title = "Dyeing Shrinkage Report - Kalash Infotech";
  document.getElementById("pageHeading").innerHTML = "Dyeing Shrinkage Report";
...
function loadValuesInViewTable(results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (results[i] instanceof Array) {
      ...
      newCell = document.createElement("td");
      var tempQtySent = parseFloat(results[i].totalQty).toFixed(2);
      newCell.textContent = results[i].totalQty;
      newRow.appendChild(newCell);

      var tempQtyReceived = parseFloat(results[i].qtyReceived) + parseFloat(results[i].totalDamage);
      newCell = document.createElement("td");
      newCell.textContent = parseFloat(tempQtyReceived).toFixed(2);
      newRow.appendChild(newCell);

      var rowShrinkage = (parseFloat(tempQtySent) - parseFloat(tempQtyReceived)).toFixed(2);
      newCell = document.createElement("td");
      newCell.style = "color: red";
      newCell.textContent = parseFloat(rowShrinkage).toFixed(2);
      newRow.appendChild(newCell);

      var rowShrinkagePercentage = ((tempQtySent - tempQtyReceived) * 100 / tempQtySent).toFixed(2);
      newCell = document.createElement("td");
      newCell.style = "color: red";
      newCell.textContent = parseFloat(rowShrinkagePercentage).toFixed(2) + " %";
      newRow.appendChild(newCell);
    }
  }
  datatableObj = dTable = $("#allPO").dataTable({
