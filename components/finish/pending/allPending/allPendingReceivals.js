...
function loadData() {
  document.title = "All Finish Pending P. O. Receivals - Kalash Infotech";
  document.getElementById("pageHeading").innerHTML = "All Finish Pending P. O. Receivals";
  ...
  connection.query(
    "SELECT * from finishpomaster where status = 'PENDING' ORDER BY billNumber desc",
...
function viewallPendingReceivalsList() {
  ...
  document.getElementById("viewPendingReceivalDetails").hidden = true;
...
function loadValuesInViewTable(results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (results[i] instanceof Array) {
      ...
      viewButton.setAttribute("onclick", "viewPendingReceivalDetails(id)");
...
function viewPendingReceivalDetails(id) {
  var temp = id.split(":");
  billNumber = temp[0];
  ...
  document.getElementById("viewPendingReceivalDetails").hidden = false;
  document.getElementById("editPoReceivalDetails").hidden = true;
