...
function loadData() {
  document.title = "All Finish Purchase P. O. - Kalash Infotech";
  document.getElementById("pageHeading").innerHTML = "All Finish Purchase P. O.";

  displayElement("waitingMessage", true);
  fetchAddresses(connection);
  viewAllFinishNewPurchaseList();
...
function viewAllFinishNewPurchaseList() {
  ...
  document.getElementById("viewNewFinishPurchaseDetails").hidden = true;
...
function loadValuesInViewTable(results) {
  var tableInstance = document.getElementById("allFinishNewPurchaseBody"),
    newRow,
    newCell;
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (results[i] instanceof Array) {
      ...
      newCell = document.createElement("td");
      newCell.textContent = results[i].totalAmount;
      newRow.appendChild(newCell);
      ...
      viewButton.setAttribute(
        "id",
        results[i].billNumber + ":" + "View"
      );
      viewButton.setAttribute("value", "View");
      viewButton.setAttribute("onclick", "viewNewFinishPurchaseDetails(id)");
      newCell.appendChild(viewButton);
      newRow.appendChild(newCell);
    }
  }

  datatableObj = $("#allFinishNewPurchase").dataTable({
...
function filterByDropdown(id) {
  ...
  var table = $('#allFinishNewPurchase').DataTable();
  switch (id) {
    ...
    case 'brokerName': table.columns(3).search(dropdownValue).draw(); break;
  }
}

function viewNewFinishPurchaseDetails(id) {
  var temp = id.split(":");
  billNumber = temp[0];
  ...
  document.getElementById("viewNewFinishPurchaseDetails").hidden = false;
  document.getElementById("editFinishPurchaseDetails").hidden = true;
  document.getElementById("backButton").hidden = false;
  document.getElementById("pageHeading").innerHTML = "View Finish Purchase P. O. Details - " + billNumber;
